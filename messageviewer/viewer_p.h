...
As the MessageViewer is now based in a theming system (Thanks to Grantlee), some default themes display an attachment list in the header.
See the HeaderTheme class for more details. The HTML code for the attachment list can not be generated by the HeaderTheme
itself, since that does not know about all attachments. Therefore, the attachment list needs to be created by ViewerPrivate.
For this, a placeholder for the attachment list is set in the HTML header of some themes. For example, the fancy theme has
this capability. Once the ObjectTreeParser is finished with the message, injectAttachments() is called. injectAttachments()
...
\par Displaying themes

The themes menu is generated dynamically with loadActionLists(). This method search through the messageviewer/themes directory
in all the KDE "data" DIRS and seek for a .desktop file in the subdirectories. If this file is found, then the appropiate information
of the theme is shown in the menu. You can change the theme at any time and the setHeaderTheme() method will create and generate the new HTML code.

\par Misc

ViewerPrivate holds the NodeHelper, which is passed on to the ObjectTreeParser when it needs it.
It also holds the HeaderTheme, AttachmentStrategy, CSSHelper, HtmlWriter and more,
