    ...
    : QWidget( parent ), mAllContactsModel( 0 ), mXmlGuiClient( guiClient ), mGrantleeThemeManager(0), mQuickSearchAction(0)
{
   ...
   *            mCheckableProxyModel:  Adds a checkbox to select the contacts
   ...
   */
    ...
    mCheckableProxyModel->setSourceModel( mContactsFilterModel );

    mItemView->setModel( mCheckableProxyModel );
...
void MainWidget::delayedInit()
{
    ...
    connect( mItemView->selectionModel(), SIGNAL(selectionChanged(QItemSelection,QItemSelection)),
             this, SLOT(updateSendMailState()));
    connect( mItemView->selectionModel(), SIGNAL(selectionChanged(QItemSelection,QItemSelection)),
             this, SLOT(slotUpdateCheckboxes(QItemSelection,QItemSelection)));
    connect( mCheckableProxyModel, SIGNAL(checkedNodesChanged()),
...
>>>>>>> 392ef66db31290ee6c13234a10ba6797aca8afc1
...
{
    for (int row = 0 ; row < mCheckableProxyModel->rowCount() ; ++row) {
        Qt::CheckState state = static_cast<Qt::CheckState>(mCheckableProxyModel->data(mCheckableProxyModel->index(row, 0), Qt::CheckStateRole).toInt());
        mItemView->selectionModel()->select(
                    mItemView->selectionModel()->model()->index(row, 0),
                    state == Qt::Checked ? QItemSelectionModel::Select : QItemSelectionModel::Deselect);
    }
}

void MainWidget::slotUpdateCheckboxes(const QItemSelection &selected, const QItemSelection &deselected)
{
    foreach (QModelIndex index, selected.indexes()) {
        if( mCheckableProxyModel->data(mCheckableProxyModel->index(index.row(), 0), Qt::CheckStateRole) != Qt::Checked) {
            mCheckableProxyModel->setData(mCheckableProxyModel->index(index.row(), 0), Qt::Checked, Qt::CheckStateRole);
        }
    }

    foreach (QModelIndex index, deselected.indexes()) {
        if( mCheckableProxyModel->data(mCheckableProxyModel->index(index.row(), 0), Qt::CheckStateRole) != Qt::Unchecked) {
            mCheckableProxyModel->setData(mCheckableProxyModel->index(index.row(), 0), Qt::Unchecked, Qt::CheckStateRole);
        }
    }
}
