  ...
  : QWidget( parent, name )
{
  ...
  lBackPixmap->setBuddy(pixmapPath);
  ...
  lBackPixmap->setEnabled(false);
  pixmapPath->setEnabled(false);

  connect (ckBackPixmap, SIGNAL(toggled(bool)), lBackPixmap, SLOT(setEnabled(bool)));
  connect (ckBackPixmap, SIGNAL(toggled(bool)), pixmapPath, SLOT(setEnabled(bool)));
