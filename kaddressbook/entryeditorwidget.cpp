...
#include <klocale.h>
#include <kapplication.h> // for kapp->palette()
#include <kglobal.h>
#include <kdebug.h>
...
void ContactDialog::ok()
{
  ...
  kdDebug() << "ContactDialog::ok() done" << endl;
}
...
// The Address/Phone group
  ...
  QTextEdit *mleAddress = new ContactMultiLineEdit( tab1, "X-BusinessAddress", ce );
  mleAddress->setWordWrap( QTextEdit::WidgetWidth );
  ...
  if (iPhone[0] == -1) kdWarning() << "X-BusinessPhone field not found" << endl;
  iPhone[1] = fieldPhone.findIndex( "X-HomePhone" );
  if (iPhone[1] == -1) kdWarning() << "X-HomePhone field not found" << endl;
  iPhone[2] = fieldPhone.findIndex( "X-BusinessFax" );
  if (iPhone[2] == -1) kdWarning() << "X-BusinessFax field not found" << endl;
  iPhone[3] = fieldPhone.findIndex( "X-MobilePhone" );
  if (iPhone[3] == -1) kdWarning() << "X-MobilePhone field not found" << endl;
  ...
  for ( int row = 0; row < numRows; row++ ) {
    ...
    QCString fieldPhoneName = (iPhone[row] != -1) ? fieldPhone[iPhone[row]].ascii() : "X-NOTFOUND";
    QLineEdit *ed = new ContactLineEdit( hGrid, fieldPhoneName, ce );
...
// The Note group
  ...
  QTextEdit *mleNotes = new ContactMultiLineEdit( tab1, "X-Notes", ce );
...
void ContactDialog::pickBirthDate()
{
  ...
  if(datePicker->exec()) {
    QString date = KGlobal::locale()->formatDate( datePicker->getDate() );
    ce->replace( "BDAY", new QString( date ) );
...
void ContactDialog::pickAnniversaryDate()
{
  ...
  if(datePicker->exec()) {
    QString date = KGlobal::locale()->formatDate( datePicker->getDate() );
    ce->replace( "X-Anniversary", new QString( date ) );
    // ce autoDelete will clean it up
  }
  delete datePicker;
}
 ...
 : QDialog( parent, "", modal ), entryField( entryField), ce( ce )
{
  ...
  // on their country code.
  //
  // This is ugly and should be changed for 3.1
