...
void LDAPSearchDialog::slotUser2()
{
  ...
  Q_FOREACH( const ContactListItem* const cli, selectedItems ) {
    ...
    if ( existing.isEmpty() ) {
      ...
      addr.setNote( i18nc( "argument is datetime", "Imported from LDAP on %1", KGlobal::locale()->formatDateTime( now ) ) );
