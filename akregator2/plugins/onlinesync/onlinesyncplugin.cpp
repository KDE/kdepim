...
K_PLUGIN_FACTORY(OnlineSyncPluginFactory,
                 registerPlugin<Akregator2::OnlineSyncPluginIface>();
)
K_EXPORT_PLUGIN(OnlineSyncPluginFactory( "akregator2_onlinesync_plugin" ) )
...
{ 
    ...
    setXMLFile( "akregator2_onlinesync_plugin.rc" , /*merge=*/true );
...
void OnlineSyncPlugin::updateActions()
{
    ...
    KAction* action;
    // Read configuration
    const KConfig config("akregator2_feedsyncrc");
