...
StartPage::StartPage( const QStringList& resources, QWidget* parent ) : QWizardPage( parent ), m_resourceBox( 0 ) {
    ...
    QLabel* label = new QLabel( i18n("Since version 1.6, Akregator2 uses the Akonadi framework to store feeds and articles. Your feeds and articles from older Akregator2 versions will now be migrated." ) );
...
void ItemImportResultPage::itemImportFinished( const Akregator2::ImportItemsJob* job, const QString& title ) {
...
class Akregator2::MigrateFeedsCommand::Private {
...
public:
    explicit Private( MigrateFeedsCommand* qq ) : q( qq ), opmlImportResultPageId( -1 ), itemImportResultPageId( -1 ) {
        opmlPath = KGlobal::dirs()->saveLocation("data", "akregator2/data") + "feeds.opml";
    }
...
void MigrateFeedsCommand::Private::startNextItemImport() {
    ...
    Akregator2::ImportItemsJob* job = new Akregator2::ImportItemsJob( weak_ptr<const NetResource>( resource ), q );
