...
class Akregator2::Backend::StorageMK4Impl::StorageMK4ImplPrivate
{
    public:
        ...
        c4_Storage* storage;
	Akregator2::Backend::StorageMK4Impl* q;
        c4_View archiveView;
        bool autoCommit;
        bool modified;
        mutable QMap<QString, Akregator2::Backend::FeedStorageMK4Impl*> feeds;
        ...
        Akregator2::Backend::FeedStorageMK4Impl* createFeedStorage( const QString& url );
};

Akregator2::Backend::StorageMK4Impl::StorageMK4Impl() : d(new StorageMK4ImplPrivate)
{
    d->q = this;
    setArchivePath(QString::null); // set path to default	//krazy:exclude=nullstrassign for old broken gcc
}

Akregator2::Backend::FeedStorageMK4Impl* Akregator2::Backend::StorageMK4Impl::StorageMK4ImplPrivate::createFeedStorage( const QString& url )
{
    if (!feeds.contains(url))
    {
        Akregator2::Backend::FeedStorageMK4Impl* fs = new Akregator2::Backend::FeedStorageMK4Impl(url, q);
        feeds[url] = fs;
        c4_Row findrow;
...
Akregator2::Backend::FeedStorage* Akregator2::Backend::StorageMK4Impl::archiveFor(const QString& url)
{
    return d->createFeedStorage( url );
}

const Akregator2::Backend::FeedStorage* Akregator2::Backend::StorageMK4Impl::archiveFor(const QString& url) const
{
    return d->createFeedStorage( url );
}

void Akregator2::Backend::StorageMK4Impl::setArchivePath(const QString& archivePath)
{
...
QString Akregator2::Backend::StorageMK4Impl::archivePath() const
{
    return d->archivePath;
}

QString Akregator2::Backend::StorageMK4Impl::defaultArchivePath()
{
    return KGlobal::dirs()->saveLocation("data", "akregator2")+"Archive";
}

Akregator2::Backend::StorageMK4Impl::~StorageMK4Impl()
{
    close();
    delete d;
    d = 0;
}
void Akregator2::Backend::StorageMK4Impl::initialize(const QStringList&) {}

bool Akregator2::Backend::StorageMK4Impl::open(bool autoCommit)
{
...
bool Akregator2::Backend::StorageMK4Impl::autoCommit() const
{
    return d->autoCommit;
}

bool Akregator2::Backend::StorageMK4Impl::close()
{
...
bool Akregator2::Backend::StorageMK4Impl::commit()
{
    QMap<QString, FeedStorageMK4Impl*>::Iterator it;
    QMap<QString, FeedStorageMK4Impl*>::Iterator end(d->feeds.end() ) ;
    for ( it = d->feeds.begin(); it != end; ++it )
...
bool Akregator2::Backend::StorageMK4Impl::rollback()
{
    QMap<QString, FeedStorageMK4Impl*>::Iterator it;
    QMap<QString, FeedStorageMK4Impl*>::Iterator end(d->feeds.end() ) ;
    for ( it = d->feeds.begin(); it != end; ++it )
...
int Akregator2::Backend::StorageMK4Impl::unreadFor(const QString &url) const
{
    c4_Row findrow;
...
void Akregator2::Backend::StorageMK4Impl::setUnreadFor(const QString &url, int unread)
{
    c4_Row findrow;
    d->purl(findrow) = url.toAscii();
    int findidx = d->archiveView.Find(findrow);
...
int Akregator2::Backend::StorageMK4Impl::totalCountFor(const QString &url) const
{
    c4_Row findrow;
    d->purl(findrow) = url.toAscii();
    int findidx = d->archiveView.Find(findrow);
...
void Akregator2::Backend::StorageMK4Impl::setTotalCountFor(const QString &url, int total)
{
    c4_Row findrow;
    d->purl(findrow) = url.toAscii();
    int findidx = d->archiveView.Find(findrow);
...
int Akregator2::Backend::StorageMK4Impl::lastFetchFor(const QString& url) const
{
    c4_Row findrow;
    d->purl(findrow) = url.toAscii();
    int findidx = d->archiveView.Find(findrow);
...
void Akregator2::Backend::StorageMK4Impl::setLastFetchFor(const QString& url, int lastFetch)
{
    c4_Row findrow;
    d->purl(findrow) = url.toAscii();
    int findidx = d->archiveView.Find(findrow);
...
void Akregator2::Backend::StorageMK4Impl::markDirty()
{
    if (!d->modified)
    {
        d->modified = true;
...
void Akregator2::Backend::StorageMK4Impl::slotCommit()
{
    if (d->modified)
        commit();
    d->modified = false;
...
QStringList Akregator2::Backend::StorageMK4Impl::feeds() const
{
    // TODO: cache list
    QStringList list;
...
void Akregator2::Backend::StorageMK4Impl::add(Storage* source)
{
...
void Akregator2::Backend::StorageMK4Impl::clear()
{
   QStringList feeds;
    int size = d->archiveView.GetSize();
    for (int i = 0; i < size; i++)
...
void Akregator2::Backend::StorageMK4Impl::storeFeedList(const QString& opmlStr)
{
...
QString Akregator2::Backend::StorageMK4Impl::restoreFeedList() const
{
    if  (d->feedListView.GetSize() == 0)
        return "";
...
void Akregator2::Backend::StorageMK4Impl::storeTagSet(const QString& xmlStr)
{
...
QString Akregator2::Backend::StorageMK4Impl::restoreTagSet() const
{
    if  (d->feedListView.GetSize() == 0)
        return "";
