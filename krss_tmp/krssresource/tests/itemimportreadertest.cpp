...
{
    QFile file( QString::fromLatin1( "%1/unittestenv/kdehome2/planetkde-correctness.xml" ).arg( QLatin1String(TESTDATAPATH) ) );
    ...
    // unread
    QVERIFY( items.contains( QLatin1String("http://labs.trolltech.com/blogs/2009/03/18/maccocoa-binary-package-available-for-testing/") ) );
    const Akonadi::Item item3 = items.value( QLatin1String("http://labs.trolltech.com/blogs/2009/03/18/maccocoa-binary-package-available-for-testing/") );
    QVERIFY( item3.flags().count() == 1 );
    QVERIFY( !item3.hasFlag( KRss::RssItem::flagRead() ) );

    // unread + important
    QVERIFY( items.contains( QLatin1String("http://www.purinchu.net/wp/?p=414") ) );
    const Akonadi::Item item4 = items.value( QLatin1String("http://www.purinchu.net/wp/?p=414") );
    QVERIFY( item4.flags().count() == 2 );
    QVERIFY( !item4.hasFlag( KRss::RssItem::flagRead() ) );
    QVERIFY( item4.hasFlag( KRss::RssItem::flagImportant() ) );

    // read
    QVERIFY( items.contains( QLatin1String("http://blusrcu.ba/nookie/?p=17") ) );
    const Akonadi::Item item5 = items.value( QLatin1String("http://blusrcu.ba/nookie/?p=17") );
    QVERIFY( item5.flags().count() == 0 );

    // read + important
    QVERIFY( items.contains( QLatin1String("tag:blogger.com,1999:blog-2333161201961104586.post-3320000861926986867") ) );
    const Akonadi::Item item6 = items.value( QLatin1String("tag:blogger.com,1999:blog-2333161201961104586.post-3320000861926986867") );
    QVERIFY( item6.flags().count() == 1 );
    QVERIFY( item6.hasFlag( KRss::RssItem::flagImportant() ) );

    // deleted
    QVERIFY( items.contains( QLatin1String("http://wm161.net/?p=467") ) );
    const Akonadi::Item item7 = items.value( QLatin1String("http://wm161.net/?p=467") );
...
{
    QFile file( QString::fromLatin1( "%1/unittestenv/kdehome2/planetkde-performance.xml" ).arg( QLatin1String(TESTDATAPATH) ) );
    if ( !file.exists() ) {
