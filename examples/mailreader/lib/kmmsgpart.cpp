...
const QTextCodec * KMMessagePart::codec() const {
  ...
  if ( !c ) {
    ...
    // the user configured
// FIXME(Andras) port to akonadi c = KMMsgBase::codecForName( GlobalSettings::self()->fallbackCharacterEncoding().toLatin1() );
  }
  if ( !c )
    // no charset means us-ascii (RFC 2045), so using local encoding should
    // be okay
// FIXME(Andras) port to akonadi c = kmkernel->networkCodec();
  assert( c );
  return c;
}
