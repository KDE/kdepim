...
//FIXME(Andras) port to akonadi #include "mailinglist-magic.h"
//FIXME(Andras) port to akonadi #include "messageproperty.h"
//FIXME(Andras) port to akonadi using KMail::MessageProperty;
#include "objecttreeparser.h"
using KMail::ObjectTreeParser;
//FIXME(Andras) port to akonadi #include "kmfolderindex.h"
//FIXME(Andras) port to akonadi #include "undostack.h"
#include "kmversion.h"
//FIXME(Andras) port to akonadi #include <version-kmail.h>
#include "kmmessagetag.h"
#include "headerstrategy.h"
//FIXME(Andras) port to akonadi #include "globalsettings.h"
using KMail::HeaderStrategy;
//FIXME(Andras) port to akonadi #include "kmaddrbook.h"
#include "kcursorsaver.h"
//FIXME(Andras) port to akonadi #include "templateparser.h"
//FIXME(Andras) port to akonadi using KMail::TemplateParser;
...
//FIXME(Andras) port to akonadi #include <kaddrbookexternal.h>
...
{
//FIXME(Andras) port to akonadi   MessageProperty::forget( this );
  delete mMsg;
...
{
  delete mMsg;
//FIXME(Andras) port to akonadi   kmkernel->undoStack()->msgDestroyed( this );
}
...
{
//FIXME(Andras) port to akonadi   MessageProperty::setSerialCache( this, newMsgSerNum );
}
...
{
//FIXME(Andras) port to akonadi   return MessageProperty::transferInProgress( getMsgSerNum() );
    return false;
}
...
//FIXME(Andras) disable, as OTP is rewritten to use KMime    otp.parseObjectTree( root );
  }
  ...
  if( curNode ) {
    ...
    ObjectTreeParser otp( 0, 0, true, false, true );
//FIXME(Andras) disable, as OTP is rewritten to use KMime    otp.parseObjectTree( curNode );
...
// FIXME(Andras) port to akonadi MailingList::name(this, headerName, mailingListStr);
  replyToStr = replyTo();
...
//#define MDN_DEBUG 1
  ...
  KConfigGroup mdnConfig;//FIXME(Andras) port to akonadi ( KMKernel::config(), "MDN" );
  ...
  QString finalRecipient;//FIXME(Andras) port to akonadi = kmkernel->identityManager()->identityForUoidOrDefault( identityUoid() ).fullEmailAddr();
...
{
    KConfigGroup general;//FIXME(Andras) port to akonadi ( KMKernel::config(), "General" );
  DwHeaders& header = mMsg->Headers();
  time_t unixTime;
