...
//FIXME(Andras) port to akonadi #include "kmfolder.h"
//FIXME(Andras) #include "kmmsgdict.h"
...
//FIXME(Andras) port to akonadi     charsetName = GlobalSettings::self()->fallbackCharacterEncoding().toLatin1();
  }
                                           ...
                                           const QByteArray& charset )
{
  ...
  if (charset.isEmpty())
  {
      //FIXME(Andras) port to akonadi cset = kmkernel->networkCodec()->name();
    kAsciiToLower(cset.data());
  }
  else cset = charset;

  const QTextCodec *codec = codecForName(cset);
  //FIXME(Andras) port to akonadi if (!codec) codec = kmkernel->networkCodec();
...
{
  int p = _str.indexOf('\'');
// FIXME(Andras) port to akonadi if (p < 0) return kmkernel->networkCodec()->toUnicode(_str);
...
QByteArray KMMsgBase::autoDetectCharset(const QByteArray &_encoding, const QStringList &encodingList, const QString &text)
{
    ...
    for (; it != charsets.constEnd(); ++it)
    {
       ...
       if (encoding == "locale")
       {
// FIXME(Andras) port to akonadi encoding = kmkernel->networkCodec()->name();
...
{
//   FIXME(Andras) port to akonadi unsigned long msn = MessageProperty::serialCache( this );
    unsigned long msn = 0;
/*FIXME(Andras) port to akonadi
    if (msn)
    return msn;
