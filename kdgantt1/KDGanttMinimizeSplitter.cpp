/* -*- Mode: C++ -*-
   $Id: KDGanttMinimizeSplitter.cpp,v 1.10 2005/10/11 13:59:02 lutz Exp $
*/
...
void KDGanttMinimizeSplitter::moveSplitter( QCOORD p, int id )
{
    ...
    bool upLeft;
    if ( QApplication::isRightToLeft() && orient == Horizontal ) {
	p += s->wid->width();
	upLeft = p > oldP;
    } else
	upLeft = p < oldP;
...
{
    if ( orient == Horizontal ) {
	if ( QApplication::isRightToLeft() && orient == Horizontal && !isSplitter )
...
void KDGanttMinimizeSplitter::getRange( int id, int *min, int *max )
{
    ...
    QRect r = contentsRect();
    if ( orient == Horizontal && QApplication::isRightToLeft() ) {
