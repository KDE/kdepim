...
void KAEventData::setAudioFile(const QString& filename, float volume, float fadeVolume, int fadeSeconds, bool allowEmptyFile)
{
	mAudioFile = filename;
...
*/
bool KAEventData::convertStartOfDay(Event* event)
{
	bool changed = false;
	QTime midnight(0, 0);
	QStringList flags = event->customProperty(KCalendar::APPNAME, FLAGS_PROPERTY).split(SC, QString::SkipEmptyParts);
