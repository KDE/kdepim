...
class ModelStateSaver::Private
{
  ...
  public:
    ...
    void saveState( const QModelIndex &index,
                    QHash<QByteArray,QVector<QPair<QString, QVariant> > >& values )
    {
...
ModelStateSaver::ModelStateSaver( QAbstractItemModel *model, QObject *parent )
  : QObject( parent ), d( new Private( model, this ) )
{
}
...
void ModelStateSaver::restoreConfig( const KConfigGroup &configGroup )
{
  Q_FOREACH ( const Role &r, d->roles ) {
    ...
    if ( l.size() % 2 != 0 ) {
      kWarning()
        << "Ignoring invalid configuration value because of odd number of list entries:" << ck;
      continue;
    }
  ...
  if ( !d->pendingProperties.isEmpty() ) {
    connect( d->model, SIGNAL(rowsInserted(QModelIndex,int,int)),
             SLOT(rowsInserted(QModelIndex,int,int)), Qt::QueuedConnection );
  }
}
...
void ModelStateSaver::addRole( int role, const QByteArray &identifier, const QVariant &defValue )
{
  Role r;
  r.role = role;
  r.identifier = identifier;
  r.defaultValue = defValue;
