...
Base::run( const char *cmd, const char *passphrase, bool onlyReadFromPGP )
{
  ...
  if(passphrase)
  {
    ...
    //kDebug( 5326 ) << "pgp PGPPASSFD = " << tmp << endl;
    //kDebug( 5326 ) << "pgp pass = " << passphrase << endl;
  }
  else
    ::unsetenv("PGPPASSFD");

  //Uncomment these lines for testing only! Doing so will decrease security!
  kDebug( 5326 ) << "pgp cmd = " << cmd << endl;
  ...
  if (!onlyReadFromPGP) {
    if (!input.isEmpty()) {
      ...
      for (int i=0; i<input.length(); i+=len2) {
        ...
        //kDebug( 5326 ) << "Polling pin[1]..." << endl;
        pollstatus = poll(&pollin, 1, 5);
        if (pollstatus == 1) {
          //kDebug( 5326 ) << "Status for polling pin[1]: " << pollin.revents << endl;
          if (pollin.revents & POLLERR) {
            kDebug( 5326 ) << "PGP seems to have hung up" << endl;
            break;
          }
            ...
            //kDebug( 5326 ) << "Trying to write " << len2 << " bytes to pin[1] ..." << endl;
            len2 = write(pin[1], input.mid(i,len2).data(), len2);
            //kDebug( 5326 ) << "Wrote " << len2 << " bytes to pin[1] ..." << endl;
          }
        }
        else if (!pollstatus) {
        ...
        if (pout[0] >= 0) {
          do {
            ...
            //kDebug( 5326 ) << "Polling pout[0]..." << endl;
            pollstatus = poll(&pollout, 1, 0);
            if (pollstatus == 1) {
              //kDebug( 5326 ) << "Status for polling pout[0]: " << pollout.revents << endl;
              if (pollout.revents & POLLIN) {
                //kDebug( 5326 ) << "Trying to read " << 1024 << " bytes from pout[0]" << endl;
                if ((len = read(pout[0],str,1024))>0) {
                  //kDebug( 5326 ) << "Read " << len << " bytes from pout[0]" << endl;
                  str[len] ='\0';
                  output += str;
                }
                else
        ...
        if (perr[0] >= 0) {
          do {
            ...
            //kDebug( 5326 ) << "Polling perr[0]..." << endl;
            pollstatus = poll(&pollerr, 1, 0);
            if (pollstatus == 1) {
              //kDebug( 5326 ) << "Status for polling perr[0]: " << pollerr.revents << endl;
              if (pollerr.revents & POLLIN) {
                //kDebug( 5326 ) << "Trying to read " << 1024 << " bytes from perr[0]" << endl;
                if ((len = read(perr[0],str,1024))>0) {
                  //kDebug( 5326 ) << "Read " << len << " bytes from perr[0]" << endl;
                  str[len] ='\0';
                  error += str;
                }
                else
      ...
      write(pin[1], "\n", 1);
    //kDebug( 5326 ) << "All input was written to pin[1]" << endl;
  }
  close(pin[1]);
  ...
  do {
    //kDebug( 5326 ) << "Checking if PGP is still running..." << endl;
    childExitStatus = 0;
    waitpidRetVal = waitpid(child_pid, &childExitStatus, WNOHANG);
    //kDebug( 5326 ) << "waitpid returned " << waitpidRetVal << endl;
    if (pout[0] >= 0) {
      do {
        // check if there is data to read from pout[0]
        //kDebug( 5326 ) << "Polling pout[0]..." << endl;
  ...
  if (WIFEXITED(childExitStatus) != 0) {
    ...
    childExitStatus = -1;
    kDebug( 5326 ) << "PGP exited abnormally!" << endl;
  }

  //Uncomment these lines for testing only! Doing so will decrease security!
  //kDebug( 5326 ) << "pgp output = " << QString(output) << endl;
  //kDebug( 5326 ) << "pgp error = " << error << endl;
...
Base::runGpg( const char *cmd, const char *passphrase, bool onlyReadFromGnuPG )
{
  ...
  if(passphrase)
  {
    ...
    //Uncomment these lines for testing only! Doing so will decrease security!
    //kDebug( 5326 ) << "pass = " << passphrase << endl;
  }
  ...
  if(!(child_pid = fork()))
  {
    ...
    kDebug( 5326 ) << "pgp cmd = " << gpgcmd << endl;
  ...
  do {
    //kDebug( 5326 ) << "Checking if GnuPG is still running..." << endl;
    childExitStatus = 0;
    waitpidRetVal = waitpid(child_pid, &childExitStatus, WNOHANG);
    //kDebug( 5326 ) << "waitpid returned " << waitpidRetVal << endl;
    do {
    ...
    if (pollstatus == -1) {
      kDebug( 5326 ) << "GnuPG poll failed, errno: " << errno << endl;
    }
  ...
  if (WIFEXITED(childExitStatus) != 0) {
    ...
    childExitStatus = -1;
    kDebug( 5326 ) << "GnuPG exited abnormally!" << endl;
  }

  //Uncomment these lines for testing only! Doing so will decrease security!
  //kDebug( 5326 ) << "gpg stdout:\n" << QString(output) << endl;
