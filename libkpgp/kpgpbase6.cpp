...
Base6::pubKeys()
{
  ...
  while( (index = error.indexOf("\n",index)) != -1 )
  {
	...
	line = error.mid(index4+7,index2-index4-7);
      ...
      //kDebug( 5326 ) << "Base: found key for " << (const char *)line << endl;
  ...
  while ( (index = error.indexOf("\n >", index)) != -1 ) {
    ...
    //kDebug( 5326 ) << "Base6: found key group for " << line << endl;
    publicKeys.append(line);
  }
...
Base6::isVersion6()
{
  ...
  {
    //kDebug( 5326 ) << "kpgpbase: pgp version 6.x detected" << endl;
    return 1;
  }

  //kDebug( 5326 ) << "kpgpbase: not pgp version 6.x" << endl;
  return 0;
}
...
// the next key's data in offset.
{
  ...
  while( true )
  {
	...
	for ( int idx = 0 ; (idx = fingerprint.indexOf(' ', idx)) != -1; )
      ...
      { // line contains an additional user id
        ...
        //kDebug( 5326 )<<"User ID: "<<output.mid( pos, eol-pos )<<endl;
  ...
  if( key != 0 )
  {
    ...
    //kDebug( 5326 )<<"Key capabilities: "<<(canEncr?"E":"")<<(canSign?"SC":"")<<endl;
  }
...
{
  kDebug( 5326 ) << "Kpgp::Base6::parseKeyList()" << endl;
  KeyList keys;
  Key *key = 0;
  int offset;
