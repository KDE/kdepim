    ...
    m_ldapSearchDialog( 0 )
{
  ...
  connect( m_availableView->selectionModel(), SIGNAL( selectionChanged( const QItemSelection&, const QItemSelection& ) ),
  ...
  connect( filterEdit, SIGNAL( textChanged( const QString& ) ),
           m_availableModel, SLOT( setFilterString( const QString& ) ) );
  ...
  m_toButton = new QPushButton( i18n( "To >>" ), this );
  buttonLayout->addWidget( m_toButton );
  connect( m_toButton, SIGNAL( clicked() ), this, SLOT( addSelectedTo() ) );

  m_ccButton = new QPushButton( i18n( "CC >>" ), this );
  buttonLayout->addWidget( m_ccButton );
  connect( m_ccButton, SIGNAL( clicked() ), this, SLOT( addSelectedCC() ) );

  m_bccButton = new QPushButton( i18n( "BCC >>" ), this );
  buttonLayout->addWidget( m_bccButton );
  connect( m_bccButton, SIGNAL( clicked() ), this, SLOT( addSelectedBCC() ) );
  ...
  connect( m_selectedView->selectionModel(), SIGNAL( selectionChanged( const QItemSelection&, const QItemSelection& ) ),
           this, SLOT( selectedSelectionChanged() ) );
  connect( m_selectedModel, SIGNAL( rowsInserted( const QModelIndex, int, int ) ),
           this, SLOT( selectedCountChanged() ) );
  connect( m_selectedModel, SIGNAL( rowsRemoved( const QModelIndex, int, int ) ),
           this, SLOT( selectedCountChanged() ) );

  m_saveButton = new QPushButton( i18n( "Save as Distribution List..." ), rightBox );
  m_saveButton->setEnabled( false );
  connect( m_saveButton, SIGNAL( clicked() ), this, SLOT( saveAsDistributionList() ) );
...
void AddressesDialog::setSelectedCC( const QStringList &contents )
{
  ...
  foreach ( const QString &content, contents ) {
    ...
    contact.setNameFromString( name );
    contact.insertEmail( email );
    addContactToSelected( contact, selectedCcItem() );
  }
}
...
void AddressesDialog::setSelectedBCC( const QStringList &contents )
{
  ...
  foreach ( const QString &content, contents ) {
    ...
    contact.setNameFromString( name );
    contact.insertEmail( email );
    addContactToSelected( contact, selectedBccItem() );
  }
}

void AddressesDialog::setShowCC( bool visible )
{
  m_ccButton->setVisible( visible );
}

void AddressesDialog::setShowBCC( bool visible )
{
  m_bccButton->setVisible( visible );
}

QStringList AddressesDialog::to() const
{
  QStringList emails = allDistributionLists( m_toItem );

  const KABC::Addressee::List contacts = toAddresses();
  emails += entryToString( contacts );

  return emails;
}

QStringList AddressesDialog::cc() const
{
  QStringList emails = allDistributionLists( m_ccItem );

  const KABC::Addressee::List contacts = ccAddresses();
  emails += entryToString( contacts );

  return emails;
}

QStringList AddressesDialog::bcc() const
{
  QStringList emails = allDistributionLists( m_bccItem );

  const KABC::Addressee::List contacts = bccAddresses();
  emails += entryToString( contacts );

  return emails;
}

KABC::Addressee::List AddressesDialog::toAddresses()  const
{
...
KABC::Addressee::List AddressesDialog::ccAddresses()  const
{
  return allAddressee( m_ccItem );
}

KABC::Addressee::List AddressesDialog::bccAddresses()  const
{
  return allAddressee( m_bccItem );
}

QStringList AddressesDialog::toDistributionLists() const
{
  return allDistributionLists( m_toItem );
}

QStringList AddressesDialog::ccDistributionLists() const
{
  return allDistributionLists( m_ccItem );
}

QStringList AddressesDialog::bccDistributionLists() const
{
  return allDistributionLists( m_bccItem );
}

void AddressesDialog::addToSelected( const Akonadi::Item &item, QStandardItem *parentItem )
{
  if ( item.hasPayload<KABC::Addressee>() )
...
void AddressesDialog::addContactToSelected( const KABC::Addressee &contact, QStandardItem *parentItem )
{
  Q_ASSERT( parentItem );
...
void AddressesDialog::addGroupToSelected( const KABC::ContactGroup &group, QStandardItem *parentItem )
{
  Q_ASSERT( parentItem );
...
QStringList AddressesDialog::entryToString( const KABC::Addressee::List &contacts ) const
{
  QStringList entries;
...
void AddressesDialog::saveAsDistributionList()
{
...
void AddressesDialog::searchLdap()
{
...
void AddressesDialog::ldapSearchResult()
{
...
KABC::Addressee::List AddressesDialog::allAddressee( QStandardItem* parent ) const
{
...
QStringList AddressesDialog::allDistributionLists( QStandardItem* parent ) const
{
  QStringList lists;
