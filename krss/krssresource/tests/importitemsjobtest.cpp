...
void ImportItemsJobTest::testCorrectness()
{
    ...
    KRssResource::ImportItemsJob *ijob = new KRssResource::ImportItemsJob( QLatin1String("http://planetkde.org/rss20.xml"), this );
    ijob->setResourceId( QLatin1String("akonadi_opml_rss_resource_0") );
    ijob->setFlagsSynchronizable( false );
    ijob->setSourceFile( kdehome + QLatin1String("/planetkde-correctness.xml") );
    QVERIFY( ijob->exec() );

    CollectionFetchJob *cjob = new CollectionFetchJob( Collection::root(), CollectionFetchJob::Recursive, this );
    cjob->setResource( QLatin1String("akonadi_opml_rss_resource_0") );
    QVERIFY( cjob->exec() );
    const QList<Collection> feeds = cjob->collections();
    QVERIFY( feeds.count() == 2 );  // root + planetkde

    Collection planetkde;
    if ( static_cast<KRss::FeedCollection>( feeds.at( 0 ) ).xmlUrl() == QLatin1String("http://planetkde.org/rss20.xml") )
        planetkde = feeds.at( 0 );
    else if ( static_cast<KRss::FeedCollection>( feeds.at( 1 ) ).xmlUrl() == QLatin1String("http://planetkde.org/rss20.xml") )
    ...
    Q_FOREACH( const Akonadi::Item& item, items ) {
        if ( item.remoteId() == QLatin1String("tag:blogger.com,1999:blog-6431293039768060590.post-962616740612527640") ) {
            ...
            QVERIFY( rssItem.title() == QLatin1String("Sam Duff (Socceroos): Odd, but cool") );
            QVERIFY( rssItem.link() == QLatin1String("http://socceroosd.blogspot.com/2009/03/odd-but-cool.html") );
            QVERIFY( rssItem.description() == QLatin1String("Content 1") );
            QVERIFY( rssItem.content().isEmpty() );
            QVERIFY( rssItem.guid() == QLatin1String("tag:blogger.com,1999:blog-6431293039768060590.post-962616740612527640") );
            //TODO: test the remaining properties
    }
    else if ( item.remoteId() == QLatin1String("http://labs.trolltech.com/blogs/2009/03/18/maccocoa-binary-package-available-for-testing/") ) {
            // unread
            QVERIFY( item.flags().count() == 1 );
            QVERIFY( !item.hasFlag( KRss::RssItem::flagRead() ) );
        }
        else if ( item.remoteId() == QLatin1String("http://wm161.net/?p=467") ) {
            // deleted
            QVERIFY( item.flags().count() == 1 );
        ...
        else if ( item.remoteId() == QLatin1String("http://blusrcu.ba/nookie/?p=17") ) {
            // read
            QVERIFY( item.flags().count() == 0 );
        }
        else if ( item.remoteId() == QLatin1String("tag:blogger.com,1999:blog-2333161201961104586.post-3320000861926986867") ) {
            // read + important
            QVERIFY( item.flags().count() == 1 );
        ...
        else if ( item.remoteId() == QLatin1String("http://www.purinchu.net/wp/?p=414") ) {
            // unread + important
            QVERIFY( item.flags().count() == 2 );
            QVERIFY( !item.hasFlag( KRss::RssItem::flagRead() ) );
            QVERIFY( item.hasFlag( KRss::RssItem::flagImportant() ) );
...
void ImportItemsJobTest::testPerformance()
{
    ...
    ijob->setResourceId( QLatin1String("akonadi_opml_rss_resource_0") );
    ijob->setFlagsSynchronizable( false );
    if ( !QFile::exists( kdehome + QLatin1String("/planetkde-performance.xml") ) ) {
        qDebug() << "Skipping the performance test";
        return;
    }

    ijob->setSourceFile( kdehome + QLatin1String("/planetkde-performance.xml") );
    QBENCHMARK {
        QVERIFY( ijob->exec() );
    }
}
