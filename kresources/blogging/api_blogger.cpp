...
void bloggerAPI::getBlogs()
{
	if ( isValid )
	{
		...
		m_xmlrpcServer->call( "blogger.getUsersBlogs", args,
          ...
          kdDebug() << "bloggerAPI::getBlogs(): Server not yet initialized. Please wait until userInfoFinished is emited"<<endl;
        }

}

void bloggerAPI::post( const blogPost &pst, bool publish )
{
	if ( isValid )
	{
		QValueList<QVariant> args;
...
void bloggerAPI::editPost( const blogPost& pst, bool publish )
{
	if ( isValid )
	{
		QValueList<QVariant> args;
...
void bloggerAPI::dumpBlog( const blogPost &blog ) 
{
  kdDebug() << "-----------------------------------" << endl;
