...
QString ADConfigDataBase::readConfigData(bool sessionStarting, bool& deletedClients, bool& deletedCalendars)
{
  ...
  for (unsigned int i = 0;  i < clients.count();  ++i)
  {
    ...
    else
    {
      ...
      info->notificationType  = clientConfig.readBoolEntry("Command line notify", false);
...
ClientInfo::ClientInfo(const QString& titl, const QString& dcopObj, int notifyType, bool disp, bool wait)
  : title(titl),
    dcopObject(dcopObj),
    displayCalName(disp),
