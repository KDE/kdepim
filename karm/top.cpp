	...
	_totalTime( 0 )
{
  ...
  // connections
  connect( _karm, SIGNAL( sessionTimeChanged( long ) ),           this,
                  SLOT( updateTime( long ) ) );
  connect( _karm, SIGNAL( currentChanged ( QListViewItem * ) ),   this,
                  SLOT( slotSelectionChanged() ));
  connect( _karm, SIGNAL( selectionChanged ( QListViewItem * ) ), this,
                  SLOT( slotSelectionChanged()) );
  connect( _karm, SIGNAL( updateButtons() ),                      this,
                  SLOT( slotSelectionChanged()) );
  connect( _karm, SIGNAL( timerTick() ),                          this,
                  SLOT( updateTime() ));
...
void KarmWindow::resetSessionTime()
{
  ...
  statusBar()->changeItem(
        i18n("This session: %1").arg( QString::fromLatin1("0:00")), 0 );
...
void KarmWindow::makeMenus()
{
  ...
  actionKeyBindings = KStdAction::keyBindings( this, SLOT( keyBindings() ),
                                               actionCollection());
  (void) KStdAction::preferences( _preferences, SLOT( showDialog() ),
                                  actionCollection());
  (void) KStdAction::save( _preferences, SLOT( save() ), actionCollection());
  actionResetSession = new KAction( i18n("&Reset Session Times"), CTRL + Key_R,
                                    this, SLOT( resetSessionTime() ),
                                    actionCollection(), "reset_session_time");
  actionStart        = new KAction( i18n("&Start"), 
                                    QString::fromLatin1("1rightarrow"),
                                    Key_S, _karm, SLOT( startTimer() ),
                                    actionCollection(), "start");
  actionStop         = new KAction( i18n("S&top"), QString::fromLatin1("stop"),
                                    Key_Escape, _karm,
                                    SLOT( stopCurrentTimer() ),
                                    actionCollection(), "stop");
  actionNew          = new KAction( i18n("New..."),
                                    QString::fromLatin1("filenew"),
                                    CTRL+Key_N, _karm, SLOT(newTask()),
                                    actionCollection(), "new_task");
  actionNewSub       = new KAction( i18n("New Subtask..."),
                                    QString::fromLatin1("kmultiple"),
                                    CTRL+ALT+Key_N, _karm, SLOT( newSubTask() ),
                                    actionCollection(), "new_sub_task");
  actionDelete       = new KAction( i18n("&Delete..."),
                                    QString::fromLatin1("editdelete"),
                                    Key_Delete, _karm, SLOT(deleteTask()),
                                    actionCollection(), "delete_task");
  actionEdit         = new KAction( i18n("&Edit..."),
                                    QString::fromLatin1("edit"),
                                    CTRL + Key_E,_karm, SLOT( editTask() ),
                                    actionCollection(), "edit_task");
  ...
  actionKeyBindings->setWhatsThis( i18n("This will let you configure "
                                        "keybindings which is specific to "
                                        "karm"));

  actionResetSession->setToolTip( i18n("Reset session time"));
  actionResetSession->setWhatsThis( i18n("This will reset the session time "
                                         "for all tasks."));
  ...
  actionStart->setWhatsThis( i18n("This will start timing for the selected "
                                  ...
                                  "a given task. This will, however, stop "
  ...
  actionDelete->setWhatsThis( i18n("This will delete the selected task and all "
                                   "its subtasks."));
