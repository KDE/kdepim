...
public:
    ...
    ~ItemPrivateData(){  qDeleteAll( childItems ); }
    ItemPrivateData *child( int row ) { return childItems.value( row ); }
    void appendChild( ItemPrivateData *item ) { childItems.append(item); }
    ItemPrivateData *removeChild( int row ) { return childItems.takeAt( row ); }
    ...
    ItemPrivateData *parent() { return parentItem; }
private:
...
QVariant FreeBusyItemModel::data( const QModelIndex& index, int role ) const
{
    ...
    switch( role ) {
        case Qt::DisplayRole: // return something to make modeltest happy
            return QString("%1 - %2").arg( KGlobal::locale()->formatDateTime( period.start() ) )
