...
public:
    ...
    {
        bool ok = false;
        int status = incidence->customProperty(QStringLiteral("FREEBUSY"), QStringLiteral("STATUS")).toInt(&ok);
...
ResourceManagement::ResourceManagement(QWidget *parent)
    : QDialog(parent)
{
    okButton->setText(i18nc("@action:button add resource to attendeelist"));
