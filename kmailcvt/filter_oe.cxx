...
void FilterOE::import(FilterInfo *info)
{
	...
	if (files.isEmpty()) {
		info->alert(name(), i18n("No Outlook Express mailboxes found in directory %1").arg(mailDir));
		return;
	}
...
void FilterOE::importMailBox(QString fileName)
{
...
void FilterOE::mbxImport(QDataStream& ds)
{
	...
	while (!ds.atEnd()) {
		...
		tmp.close();
		kmailMessage((FilterInfo *) inf, folderName, tmp.name());
		tmp.unlink();
	}
}
...
void FilterOE::dbxReadEmail(QDataStream& ds, int filePos)
{
	...
	tmp.close();
	kmailMessage((FilterInfo *) inf, folderName, tmp.name());
	inf->current((((float) ++currentEmail)/((float) totalEmails))*100.0);
	tmp.unlink();
