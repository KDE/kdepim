...
pab::pab(const char *_pabfile,filter *_f,filterInfo *_info)
{
  ...
  info=_info;
  in=fopen(pabfile,"rb");
  cap=i18n("Import MS-Exchange Personal Adressbook (.PAB)");
}
...
bool ret;
   ...
   if (in==NULL) {QString msg;
     msg.sprintf(" '%s' ",pabfile);
     msg=i18n("Can't open ")+msg+i18n(" for reading");
     info->alert(cap,msg);
     return false;
   }
   if (!knownPAB()) {
...
content_t id;
   ...
   if (id!=PAB_FILE_ID) {QString msg;
     msg.sprintf("'%s' ",pabfile);
     msg=msg+i18n("has no PAB id that I know of, can't convert this");
     info->alert(cap,msg);
     return false;
   }
return true;
