...
// CALSYS: Added calType and calendarSystem stuff for different calendar types support
KDatePicker::KDatePicker(const QString& calType, QWidget *parent, QDate dt, const char *name)
  : QFrame(parent,name),
    ...
    val(new KDateValidator(calType, this)),
    table(new KDateTable(calType, this)),
    fontsize(10)
{
  kdDebug(5400) << "Iniciado";
  // -----
  setFontSize(10);
  ...
  kdDebug(5400) << "KDatePicker with different calendar systems support" << endl;
...
{
  kdDebug(5400) << "KDatePicker::tableClickedSlot: table clicked." << endl;
...
KDatePicker::setDate(const QDate& date)
{
	...
	kdDebug(5400) << "KDatePicker::setDate: refusing to set invalid date." << endl;
	return false;
    }
}
...
KDatePicker::selectMonthClicked()
{
  ...
  // CALSYSKDateInternalMonthPicker* picker = new KDateInternalMonthPicker(fontsize, popup);
  KDateInternalMonthPicker* picker = new KDateInternalMonthPicker( fontsize, popup, year, calendarSystem );
  ...
  if(popup->exec(selectMonth->mapToGlobal(QPoint(0, selectMonth->height()))))
    {
      ...
      // CALSYS
      calendarSystem->constructDateInMonth(date, month);
      //day=date.day();
...
KDatePicker::selectYearClicked()
{
  ...
  // CALSYS
  KDateInternalYearSelector* picker = new KDateInternalYearSelector(fontsize, calendarSystem, popup);
  ...
  if(popup->exec(selectYear->mapToGlobal(QPoint(0, selectMonth->height()))))
    {
      ...
      //CALSYS
     calendarSystem->constructDateInYear(date, year);
      //day=date.day();
      // ----- construct a valid date in this month:
...
KDatePicker::lineEnterPressed()
{
  ...
  if(val->date(line->text(), temp)==QValidator::Acceptable)
    {
	kdDebug(5400) << "KDatePicker::lineEnterPressed: valid date entered." << endl;
	emit(dateEntered(temp));
	setDate(temp);
    } else {
      KNotifyClient::beep();
      kdDebug(5400) << "KDatePicker::lineEnterPressed: invalid date entered." << endl;
    }
}
