...
{
   // CALSYS
  QDate tmp = calendarSystem->parseDate(text);
...
KDateTable::keyPressEvent( QKeyEvent *e )
{
    ...
    // CALSYS
    if(firstday+numdays<defPos+dayoff)
    ...
    //QDate tempDate = date.addDays(pos-temp);
    setDate(date.addDays(pos-temp));

    updateCell(temp/7+1, temp%7); // Update the previously selected cell
    updateCell(pos/7+1, pos%7); // Update the selected cell
    // CALSYS assert(QDate(date.year(), date.month(), pos-firstday+dayoff).isValid());
    assert(date.addDays(pos-temp).isValid());
...
KDateTable::contentsMousePressEvent(QMouseEvent *e)
{
  ...
  // CALSYS setDate(QDate(date.year(), date.month(), pos-firstday+dayoff));
  QDate newDate = date.addDays(difdays);
  setDate(QDate(newDate.year(), newDate.month(), newDate.day()));
...
KDateTable::setDate(const QDate& date_)
{
  ...
  firstday = calendarSystem->dayOfTheWeek(date_);

  if(firstday==1) firstday=8;

  // CALSYS numdays=date.daysInMonth();
  numdays = calendarSystem->numberOfDaysInMonth(date_);
  ...
  numDaysPrevMonth = calendarSystem->numberOfDaysPrevMonth(date_);
...
// CALSYS added year and calendarSystem parameter
...
// CALSYS added calendarSystem parameter
...
KDateInternalYearSelector::yearEnteredSlot()
{
  ...
  // CALSYS date.setYMD(year, 1, 1);
  date.setYMD(1998, 1, 1); // just a valid year and first month;
