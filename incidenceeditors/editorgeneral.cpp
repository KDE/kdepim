...
EditorGeneral::EditorGeneral( QObject *parent )
...
void EditorGeneral::initHeader( QWidget *parent, QBoxLayout *topLayout )
{
  mParent = parent;
...
void EditorGeneral::initSecrecy( QWidget *parent, QBoxLayout *topLayout )
{
...
void EditorGeneral::initDescription( QWidget *parent, QBoxLayout *topLayout )
{
...
void EditorGeneral::initAlarm( QWidget *parent, QBoxLayout *topLayout )
{
...
void EditorGeneral::initAttachments( QWidget *parent, QBoxLayout *topLayout )
{
  mAttachments = new EditorAttachments( KDialog::spacingHint(), parent );
  connect( mAttachments, SIGNAL(openURL(const KUrl &)),
           this, SIGNAL(openURL(const KUrl &)) );
  topLayout->addWidget( mAttachments, 1 );
}

void EditorGeneral::setType( const QByteArray &type )
{
  // must be "Event", "Todo", "Journal", etc.
  mType = type;
}

void EditorGeneral::addAttachments( const QStringList &attachments,
...
void EditorGeneral::selectCategories()
{
...
void EditorGeneral::editAlarms()
{
  ...
  QPointer<EditorAlarms> dlg = new EditorAlarms( mType, &mAlarmList, mAlarmEditButton );
...
void EditorGeneral::enableAlarm( bool enable )
{
  mAlarmStack->setEnabled( enable );
  mAlarmEditButton->setEnabled( enable );
}

void EditorGeneral::toggleAlarm( bool on )
{
  mAlarmButton->setChecked( on );
}

void EditorGeneral::setCategories( const QStringList &categories )
{
  mCategoriesLabel->setText( categories.join( "," ) );
  mCategories = categories;
}

void EditorGeneral::setDefaults( bool allDay )
{
  Q_UNUSED( allDay );
...
void EditorGeneral::updateDefaultAlarmTime()
{
...
void EditorGeneral::updateAlarmWidgets()
{
...
void EditorGeneral::setSummary( const QString &text )
{
  mSummaryEdit->setText( text );
}

void EditorGeneral::setDescription( const QString &text, bool isRich )
{
...
QObject *EditorGeneral::typeAheadReceiver() const
{
  return mSummaryEdit;
}

void EditorGeneral::toggleDescriptionRichButtons( bool rich )
{
  mDescriptionEdit->setActionsEnabled( rich );
}

void EditorGeneral::setDescriptionRich( bool rich )
{
...
void EditorGeneral::updateAttendeeSummary( int count )
{
  if ( count <= 0 ) {
...
bool EditorGeneral::validateInput()
{
