...
EditorGeneralTodo::EditorGeneralTodo( QObject *parent )
    : EditorGeneral( parent )
{
  setType( "Todo" );
}

EditorGeneralTodo::~EditorGeneralTodo()
{
}

void EditorGeneralTodo::finishSetup()
{
...
void EditorGeneralTodo::initTime( QWidget *parent, QBoxLayout *topLayout )
{
...
void EditorGeneralTodo::initCompletion( QWidget *parent, QBoxLayout *topLayout )
{
...
void EditorGeneralTodo::initPriority( QWidget *parent, QBoxLayout *topLayout )
{
...
void EditorGeneralTodo::initStatus( QWidget *parent, QBoxLayout *topLayout )
{
...
void EditorGeneralTodo::setDefaults( const QDateTime &due, bool allDay )
{
...
void EditorGeneralTodo::readTodo( const Todo::Ptr &todo, const QDate &date, bool tmpl )
{
  //TODO: do something with tmpl
  Q_UNUSED( tmpl );

  EditorGeneral::readIncidence( todo );
...
void EditorGeneralTodo::fillTodo( Todo::Ptr &todo )
{
  Incidence::Ptr inc = todo.staticCast<Incidence>();
  EditorGeneral::fillIncidence( inc );
...
void EditorGeneralTodo::enableDueEdit( bool enable )
{
...
void EditorGeneralTodo::enableStartEdit( bool enable )
{
...
void EditorGeneralTodo::enableTimeEdits( bool enable )
{
  if( mStartCheck->isChecked() ) {
...
void EditorGeneralTodo::showAlarm()
{
  enableAlarm( mDueCheck->isChecked() );
}

bool EditorGeneralTodo::validateInput()
{
  if ( mDueCheck->isChecked() ) {
...
void EditorGeneralTodo::updateRecurrenceSummary( const Todo::Ptr &todo )
{
  if ( todo->recurs() ) {
    mRecEditLabel->setText( KCalUtils::IncidenceFormatter::recurrenceString( todo ) );
  } else {
    mRecEditLabel->setText( QString() );
  }
}

void EditorGeneralTodo::completedChanged( int index )
{
  if ( index == 10 ) {
...
void EditorGeneralTodo::dateChanged()
{
...
void EditorGeneralTodo::startDateModified()
{
...
void EditorGeneralTodo::setCompletedDate()
{
  if ( mCompletedCombo->currentIndex() == 10 && mCompleted.isValid() ) {
...
bool EditorGeneralTodo::setAlarmOffset( const Alarm::Ptr &alarm, int value ) const
{
