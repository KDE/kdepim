...
bool EventEditor::processInput()
{
  ...
  if ( Akonadi::hasEvent( mIncidence ) ) {
    ...
    if ( *event == *oldEvent ) {
      ...
      if ( mIsCounter ) {
        Q_ASSERT( mIncidence.hasPayload<KCalCore::Event::Ptr>() );
        Akonadi::Groupware::instance()->sendCounterProposal( oldEvent, event );
    ...
    newEvent->setOrganizer( Person::Ptr( new Person( EditorConfig::instance()->fullName(),
...
{
  KCalCore::Event::Ptr event = Akonadi::event(item);
  mGeneral->fillEvent( event );
    Incidence::Ptr i = event.staticCast<Incidence>();
  if ( mFreeBusy ) {
#ifdef HAVE_QT3SUPPORT
    mFreeBusy->fillIncidence( i );
#endif
  }
  cancelRemovedAttendees( item );
  mRecurrence->fillIncidence( i );
  writeDesignerFields( i );
}
