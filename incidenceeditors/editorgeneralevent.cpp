...
EditorGeneralEvent::EditorGeneralEvent( QObject *parent )
...
EditorGeneralEvent::~EditorGeneralEvent()
{
}

void EditorGeneralEvent::finishSetup()
{
...
void EditorGeneralEvent::initTime( QWidget *parent, QBoxLayout *topLayout )
{
  ...
  if ( !EditorConfig::instance()->showTimeZoneSelectorInIncidenceEditor() ) {
...
void EditorGeneralEvent::initClass( QWidget *parent, QBoxLayout *topLayout )
{
  QBoxLayout *classLayout = new QHBoxLayout();
...
void EditorGeneralEvent::initInvitationBar( QWidget *parent, QBoxLayout *layout )
{
...
void EditorGeneralEvent::setTimeEditorsEnabled( bool enabled )
{
...
void EditorGeneralEvent::slotHasTimeCheckboxToggled( bool checked )
{
...
void EditorGeneralEvent::setDateTimes( const QDateTime &start, const QDateTime &end )
{
  setDateTimes( KDateTime( start, KSystemTimeZones::local() ),
                KDateTime( end, KSystemTimeZones::local() ) );
}

void EditorGeneralEvent::setDateTimes( const KDateTime &start, const KDateTime &end )
{
...
void EditorGeneralEvent::setTimes( const QDateTime &start, const QDateTime &end )
{
  setDateTimes( KDateTime( start, KSystemTimeZones::local() ),
                KDateTime( end, KSystemTimeZones::local() ) );
}

void EditorGeneralEvent::setTimes( const KDateTime &start, const KDateTime &end )
{
...
void EditorGeneralEvent::startTimeChanged( const QTime &newtime )
{
...
void EditorGeneralEvent::endTimeChanged( const QTime &newtime )
{
...
void EditorGeneralEvent::startDateChanged( const QDate &newdate )
{
  if ( !newdate.isValid() ) {
    return;
  }
...
void EditorGeneralEvent::endDateChanged( const QDate &newdate )
{
  if ( !newdate.isValid() ) {
    return;
  }
...
void EditorGeneralEvent::startSpecChanged()
{
...
void EditorGeneralEvent::endSpecChanged()
{
  mEndSpec = mTimeZoneComboEnd->selectedTimeSpec();
...
void EditorGeneralEvent::setDefaults( const QDateTime &from,
                                        const QDateTime &to, bool allDay )
{
  EditorGeneral::setDefaults( allDay );
...
void EditorGeneralEvent::readEvent( const Event::Ptr &event, const QDate &date, bool isTemplate )
{
  ...
  const QStringList allEmails = EditorConfig::instance()->allEmails();
  Attendee::Ptr me = event->attendeeByMails( allEmails );
...
void EditorGeneralEvent::fillEvent( Event::Ptr &event )
{
  Incidence::Ptr inc = event.staticCast<Incidence>();
  fillIncidence( inc );
  ...
  event->setTransparency( mFreeTimeCombo->currentIndex() > 0 ?
                          KCalCore::Event::Transparent : KCalCore::Event::Opaque );
}

void EditorGeneralEvent::setDuration()
{
...
void EditorGeneralEvent::emitDateTimeStr()
{
...
bool EditorGeneralEvent::validateInput()
{
  if ( mHasTimeCheckbox->isChecked() ) {
  ...
  return EditorGeneral::validateInput();
}

bool EditorGeneralEvent::setAlarmOffset( const Alarm::Ptr &alarm, int value ) const
{
  alarm->setStartOffset( value );
  return true;
}

void EditorGeneralEvent::updateRecurrenceSummary( const Event::Ptr &event )
{
  if ( event->recurs() ) {
    mRecEditLabel->setText( KCalUtils::IncidenceFormatter::recurrenceString( event ) );
  } else {
    mRecEditLabel->setText( QString() );
  }
}

#include "editorgeneralevent.moc"
