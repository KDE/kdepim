...
void EditorGeneralEvent::readEvent( Event *event, const QDate &date, bool isTemplate )
{
...
void EditorGeneralEvent::fillEvent( Event *event )
{
  fillIncidence( event );
...
bool EditorGeneralEvent::setAlarmOffset( Alarm *alarm, int value ) const
{
  alarm->setStartOffset( value );
  return true;
}

void EditorGeneralEvent::updateRecurrenceSummary( Event *event )
{
  if ( event->recurs() ) {
