...
EditorRecurrence::EditorRecurrence( QWidget *parent ) : QWidget( parent )
{
  mParent = parent;
...
EditorRecurrence::~EditorRecurrence()
{
}
...
void EditorRecurrence::showCurrentRule( int current )
{
  switch ( current ) {
  case Daily:
...
void EditorRecurrence::setDateTimes( const QDateTime &start, const QDateTime &end )
{
  mEventStartDt = start;
...
void EditorRecurrence::setDefaults( const QDateTime &from, const QDateTime &to, bool )
{
...
void EditorRecurrence::readIncidence( const Incidence::Ptr &incidence )
{
  if ( !incidence ) {
    return;
  }
  ...
  if ( incidence->type() == IncidenceBase::TypeTodo ) {
    ...
    setDefaults( incidence->dtStart().toTimeSpec( timeSpec ).dateTime(),
                 incidence->dateTime( Incidence::RoleEndRecurrenceBase ).toTimeSpec( timeSpec ).dateTime(), incidence->allDay() );
  }
...
void EditorRecurrence::fillIncidence( Incidence::Ptr &incidence )
{
...
void EditorRecurrence::setDateTimeStr( const QString &str )
{
  mDateTimeLabel->setText( str );
}

bool EditorRecurrence::validateInput()
{
  // Check input here.
...
void EditorRecurrence::showExceptionsDialog()
{
...
void EditorRecurrence::showRecurrenceRangeDialog()
{
  int duration = mRecurrenceRange->duration();
...
bool EditorRecurrence::recurs()
{
  return mEnabledCheck->isChecked();
}

void EditorRecurrence::saveValues()
{
  int duration = mRecurrenceRange->duration();
  QDate endDate;
  if ( duration == 0 ) {
...
void EditorRecurrence::restoreValues()
{
  QBitArray rDays( 7 );
  int day = 0;
  int count = 0;
  ...
  if ( mSaveRec.startDateTime().isValid() && mSaveRec.endDateTime().isValid() ) {
    KDateTime::Spec timeSpec = KSystemTimeZones::local();
    setDefaults( mSaveRec.startDateTime().toTimeSpec( timeSpec ).dateTime(),
                 mSaveRec.endDateTime().toTimeSpec( timeSpec ).dateTime(),
                 mSaveRec.allDay() );
  }
  ...
  case Recurrence::rMonthlyPos:
  {
    ...
    QList<RecurrenceRule::WDayPos> rmp = mSaveRec.monthPositions();
    if ( !rmp.isEmpty() ) {
      mMonthly->setByPos( rmp.first().pos(), rmp.first().day() );
    }
  ...
  case Recurrence::rMonthlyDay:
  {
    ...
    QList<int> rmd = mSaveRec.monthDays();
    // check if we have any setting for which day (vcs import is broken and
    // does not set any day, thus we need to check)
    if ( !rmd.isEmpty() ) {
      day = rmd.first();
  ...
  case Recurrence::rYearlyMonth:
  {
    ...
    QList<int> rmd = mSaveRec.yearDates();
    if ( !rmd.isEmpty() ) {
      day = rmd.first();
    }
  ...
  case Recurrence::rYearlyPos:
  {
    ...
    QList<int> months = mSaveRec.yearMonths();
    if ( !months.isEmpty() ) {
      month = months.first();
    }
    QList<RecurrenceRule::WDayPos> pos = mSaveRec.yearPositions();
    if ( !pos.isEmpty() ) {
      count = pos.first().pos();
      day = pos.first().day();
    }
  ...
  case Recurrence::rYearlyDay:
  {
    ...
    QList<int> days = mSaveRec.yearDays();
    if ( !days.isEmpty() ) {
      day = days.first();
    }
    if ( day > 0 ) {
  ...
  if ( mSaveRec.startDateTime().isValid() ) {
    KDateTime::Spec timeSpec = KSystemTimeZones::local();
    mRecurrenceRange->setDateTimes( mSaveRec.startDateTime().toTimeSpec( timeSpec ).dateTime() );
  }
...
EditorRecurrenceDialog::EditorRecurrenceDialog( QWidget *parent )
  : KDialog( parent )
{
  setModal( false );
