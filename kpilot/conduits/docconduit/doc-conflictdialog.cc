...
ResolutionDialog::ResolutionDialog( QWidget* parent, const QString& caption, syncInfoList*sinfo, KPilotDeviceLink*lnk )
	...
	resolutionGroupBox->setColumnLayout(0, Qt::Vertical );
	resolutionGroupBoxLayout = new QGridLayout( resolutionGroupBox->layout() );
	...
	if (syncInfo) {
		...
		for (it=(*syncInfo).begin(); it!=(*syncInfo).end(); it++) {
			...
			cE.dbname=new QLabel(text, resolutionGroupBox);
			resolutionGroupBoxLayout->addWidget( cE.dbname, cE.index, 0 );
			
			cE.resolution=new QComboBox( FALSE, resolutionGroupBox);
