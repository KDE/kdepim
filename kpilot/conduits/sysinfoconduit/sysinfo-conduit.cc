...
void SysInfoConduit::writeFile()
{
	...
	{
		case eOutputText:
			templatefile=KStandardDirs::locate("data", CSL1("kpilot/sysinfoconduit/Template.txt"));
			break;
		...
		case eOutputHTML:
		default:
			templatefile=KStandardDirs::locate("data", CSL1("kpilot/sysinfoconduit/Template.html"));
			break;
	}
	...
	{
		output.replace(CSL1("#%1#").arg(it.key()), it.value());
	}

	// Insert the list of databases
	QRegExp re(CSL1("#dblist\\[(.*)\\]#"));
	re.setMinimal(true);
	while (re.indexIn(output)>=0)
	{
		QString dbstring;
