...
[
AC_PATH_PROG(MOC, moc, /usr/bin/moc, $QTDIR/bin:/usr/bin:/usr/X11R6/bin:/usr/lib/qt/bin:/usr/local/qt/bin:/usr/lib/qt-1.44/bin:$PATH)
])
...
qt_incdirs="$ac_qt_includes /usr/lib/qt/include /usr/local/qt/include /usr/include/qt /usr/include /usr/X11R6/include/X11/qt $x_includes $QTINC /usr/lib/qt-1.44/include"
test -n "$QTDIR" && qt_incdirs="$QTDIR/include $QTDIR $qt_incdirs"
AC_FIND_FILE(qmovie.h, $qt_incdirs, qt_incdir)
ac_qt_includes=$qt_incdir

qt_libdirs="$ac_qt_libraries /usr/lib/qt/lib /usr/local/qt/lib /usr/lib/qt /usr/X11R6/lib /usr/lib $x_libraries $QTLIB /usr/lib/qt-1.44/lib"
...
if test "$ac_qt_includes" = NO || test "$ac_qt_libraries" = NO; then
  ...
  AC_MSG_ERROR([QT-1.4 or greater $ac_qt_notfound not found. Please check your installation! ]);
else
  have_qt="yes"
fi
])
...
AC_DEFUN(PILOTLINK,
[
AC_MSG_CHECKING(for pilot-link)
   AC_CACHE_VAL(ac_cv_have_pilotlink,
   [
    AC_LANG_CPLUSPLUS
    ac_save_ldflags=$LDFLAGS
    LDFLAGS="$LDFLAGS -lpicc -lpisock $LIBSOCKET"
    ...
    if test "$ac_cv_have_pilotlink" = "no"; then
      AC_MSG_ERROR([
Cannot find pilot-link and its headers.  Please check that you have
installed pilot-link (libpisock and libpicc) and the headers
(pi-file.h, etc).  You may have to run ldconfig after installing. 
See http://www.slac.com/pilone/kpilot_home/ for more information.
])
fi
    LDFLAGS=$ac_save_ldflags
   ])dnl
...
dnl This appears to be missing from some distributions
