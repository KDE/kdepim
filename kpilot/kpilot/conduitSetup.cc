...
// Space between rows in the dialog.
//
//
#define SPACING	(10)

static char *filename="CConduitSetup::";
  ...
  : QDialog(parent,name,TRUE) //,TRUE,WStyle_Customize | WStyle_NormalBorder )
{
  ...
  setCaption("Conduit Setup Dialog");
  resize(451,324);
  
  label1 = new QLabel(this, "label1");
  label1->setText("Available Conduits:");
  ...
  label2->setText("Installed Conduits:");
  ...
  fInstallConduit->setText(klocale->translate("Install"));
  ...
  fRemoveConduit->setText(klocale->translate("Uninstall"));
...
CConduitSetup::slotDone()
{
  ...
  // Unfortunetaly we need to rewrite the whole file after a conduit setup, since
  // we don't know what used to be in there and there's no deleteEntry() in KSimpleConfig.
  ...
  while(iter.current())
    {
      conduit = popen(conduitPath + "/" + iter.current() + " -info", "r");
      if(conduit)
...
CConduitSetup::slotSetupConduit()
{
  ...
  if(fSetupConduitProcess.isRunning())
    {
      KMsgBox::message(0L, klocale->translate("Setup already in progress."), 
		       klocale->translate("A conduit is already being setup up.\nPlease complete that setup before starting another."),
		       KMsgBox::STOP);
      return;
    }
  QString conduitName =   kapp->kde_datadir() + "/kpilot/conduits/";
  conduitName = conduitName + fInstalledConduits->text(fInstalledConduits->currentItem());
