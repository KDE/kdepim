...
static const char *kpilotoptions_id =
	"$Id$";
	...
	setupDialogPage(i18n("DB Specials"), p)
{
	...
	fBackupOnly->setText(c.readEntry("BackupForSync", "Arng,PmDB,lnch"));
	fSkipDB->setText(c.readEntry("SkipSync", "AvGo"));
...
/* static */ QString KPilotOptionsAddress::fGroupName = QString("Address Widget");
	...
	setupDialogPage(i18n("Address"), w)
{
	...
	currentLabel = new QLabel(i18n("Import Format:"), formatGroup);
	...
	currentLabel = new QLabel(i18n("Export Format:"), formatGroup);
	currentLabel->adjustSize();
	...
	fUseKeyField = new QCheckBox(i18n("Use &Key Field"), formatGroup);
	...
	displayGroup = new QVButtonGroup(i18n("Address Display"), this, "bg");

	fNormalDisplay = new QRadioButton(i18n("Last,First"), displayGroup);
...
void KPilotOptionsAddress::setRadio(int s)
{
	...
	{
	case 0:
		fNormalDisplay->setChecked(1);
		break;
	case 1:
		fCompanyDisplay->setChecked(1);
		break;
	default:
		fNormalDisplay->setChecked(1);
		break;
...
int KPilotOptionsAddress::getRadio() const
{
	...
	if (fNormalDisplay->isChecked())
		return 0;
	if (fCompanyDisplay->isChecked())
		return 1;
...
fPilotPort(QString::null), fFullUserName(QString::null)
{
	FUNCTIONSETUP;
	...
	fPilotDevice->setText(config.readEntry("PilotDevice", fPilotPort));
	...
	value = config.readNumEntry("PilotSpeed", fPilotRate);

	DEBUGKPILOT << fname << ": Read pilot speed "
	...
	fUserName->setText(config.readEntry("UserName", fFullUserName));
	...
	QToolTip::add(fUserName,
		i18n("You can enter your name "
		       "(as it appears on the Palm) here."));
	...
	fStartDaemonAtLogin =
		new QCheckBox(i18n("Start Hot-Sync Daemon at login. "), this);
	fStartDaemonAtLogin->adjustSize();
	fStartDaemonAtLogin->setChecked(config.
		readBoolEntry("StartDaemonAtLogin", false));
	...
	fDockDaemon = new QCheckBox(i18n("Show Daemon in Kicker."), this);
...
int KPilotOptionsGeneral::commitChanges(KConfig & config)
{
	...
	config.writeEntry("DockDaemon", (bool) fDockDaemon->isChecked());

	KPilotConfig::updateConfigVersion();

	if (fStartDaemonAtLogin->isChecked())
	{
		DEBUGKPILOT << fname << ": Copying autostart file" << endl;
...
void KPilotOptionsGeneral::getEnvironmentPilot()
{
	FUNCTIONSETUP;
	...
	fPilotPort = pilotPort;
	fPilotRate = pilotRate;
}

void KPilotOptionsGeneral::getEnvironmentUser()
{
	FUNCTIONSETUP;
...
setupDialogPage(i18n("Sync"), s)
{
	...
	fOverwriteRemote->setChecked(config.readBoolEntry("OverwriteRemote",
			false));
	grid->addWidget(fOverwriteRemote, 1, 1);
	QToolTip::add(fOverwriteRemote,
		i18n("Nobody knows what this option is for."));

	fForceFirstTime =
		new QCheckBox(i18n("Force first-time sync every time"), this);
	fForceFirstTime->setChecked(config.readBoolEntry("ForceFirst",
			false));
	grid->addWidget(fForceFirstTime, 2, 1);
	QToolTip::add(fForceFirstTime,
		i18n("Nobody knows what this option is for."));

	fSyncLastPC =
		new QCheckBox(i18n("Do full backup when changing PCs"), this);
	fSyncLastPC->setChecked(config.readBoolEntry("SyncLastPC", true));
	grid->addWidget(fSyncLastPC, 3, 1);
	QToolTip::add(fSyncLastPC,
		i18n("Nobody knows what this option is for."));

	fPreferFastSync =
		new QCheckBox(i18n("Prefer FastSync to HotSync"), this);
	fPreferFastSync->setChecked(config.readBoolEntry("PreferFastSync",
			false));
...
/* virtual */ int KPilotOptionsSync::commitChanges(KConfig & c)
{
	...
	c.writeEntry("OverwriteRemote", (bool) fOverwriteRemote->isChecked());
...
setupDialog(parent, QString::null, i18n("KPilot Options"),
...
int main(int argc, char **argv)
{
	...
	a->addAuthor("Dan Pilone",
		I18N_NOOP("Project Leader"), "pilone@slac.com");
	a->addAuthor("Adriaan de Groot",
		I18N_NOOP("KDE1 Maintainer"),
		"adridg@cs.kun.nl", "http://www.cs.kun.nl/~adridg/kpilot/");
	a->addCredit("Gregory Stern",
		I18N_NOOP("Kab2 & Abbrowser Support"), "gstern@slac.com");
	a->addCredit("Preston Brown",
		I18N_NOOP("KOrganizer Support"), "pbrown@kde.org");
	a->addCredit("Andreas Silberstorff", I18N_NOOP(".de localization"));
	a->addCredit("Nicolas Pettiaux", I18N_NOOP(".fr localization"));
	a->addCredit("Chuck Robey", I18N_NOOP("BSD porting"));
	a->addCredit("Christopher Molnar",
		I18N_NOOP("Moral and Mandrake Support"));
	a->addCredit("Scott Presnell", I18N_NOOP("Bug Fixes"));
