...
bool IDMapperXml::open()
{
	...
	if( !fFile.exists() )
	{
		DEBUGKPILOT << fname << ": Creating " << fFile.fileName() << endl;
		...
		{
			DEBUGKPILOT << fname << ": Could not create " << fFile.fileName() << endl;
			return false;
		}
	}
	else
	{
		DEBUGKPILOT << fname << ": Parsing file " << fFile.fileName() << endl;
		QXmlSimpleReader reader;
		reader.setContentHandler( this );
		
		// Make sure that the file is closed after parsing.
		const QXmlInputSource * source = new QXmlInputSource(fFile);
...
void IDMapperXml::save()
{
	...
	Q3ValueList<IDMapping>::const_iterator it;
	for ( it = fMappings.begin(); it != fMappings.end(); ++it )
	{
		...
		if( mapping.pid() != 0 )
		{
			...
			uidElement.setAttribute( CSL1("value"), static_cast<qulonglong>(mapping.pid()));
			mappingElement.appendChild( uidElement );
		}
...
Q3ValueList<IDMapping>& IDMapperXml::mappings()
{
	return fMappings;
}
