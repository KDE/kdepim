...
void ImportMailJob::restoreTransports()
{
    ...
    if (!mFileList.contains(path)) {
        ...
        if (transport && transport->isFile()) {
            ...
            const QStringList transportList = transportConfig->groupList().filter(QRegExp(QStringLiteral("Transport \\d+")));
...
void ImportMailJob::restoreConfig()
{
    ...
    if (!mFileList.contains(filtersPath)) {
        ...
        if (filter && filter->isFile()) {
            ...
            const QStringList filterList = filtersConfig->groupList().filter(QRegExp(QStringLiteral("Filter #\\d+")));
...
void ImportMailJob::restoreIdentity()
{
    ...
    if (!mFileList.contains(path)) {
        ...
        if (identity && identity->isFile()) {
            ...
            const QStringList identityList = identityConfig->groupList().filter(QRegExp(QStringLiteral("Identity #\\d+")));
...
void ImportMailJob::importFolderArchiveConfig(const KArchiveFile *archiveconfiguration, const QString &archiveconfigurationrc, const QString &filename, const QString &prefix)
{
    ...
    const QStringList archiveList = archiveConfig->groupList().filter(QRegExp(QStringLiteral("FolderArchiveAccount ")));
        ...
        if (!path.isEmpty()) {
            ...
            if (id != -1) {
                ...
                if (!QDir(path.path()).exists()) {
                    newGroup.writeEntry(QStringLiteral("storePath"), QUrl(QDir::homePath()));
                }
            }
