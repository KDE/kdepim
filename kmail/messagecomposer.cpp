...
void MessageComposer::applyChanges( bool disableCrypto )
{
  ...
  if ( !qgetenv( "KMAIL_DEBUG_COMPOSER_CRYPTO" ).isEmpty() ) {
    ...
    kDebug() << "KMAIL_DEBUG_COMPOSER_CRYPTO = TRUE";
  } else {
    mDebugComposerCrypto = false;
    kDebug() << "KMAIL_DEBUG_COMPOSER_CRYPTO = FALSE";
  }
                                             ...
                                             Kleo::CryptoMessageFormat format )
{
  ...
  if ( !mAttachments.empty() &&
    ...
    kDebug() << "Set top level Content-Type to Multipart/Mixed";
    ...
    if ( !ourFineBodyPart.originalContentTypeStr().isEmpty() ) {
      ...
      kDebug() << "Set top level Content-Type from originalContentTypeStr()="
      ...
      kDebug() << "Set top level Content-Type to"
                   << ourFineBodyPart.typeStr() << "/" << ourFineBodyPart.subtypeStr();
    }
    ...
    if ( mDebugComposerCrypto ) {
      kDebug() << "Top level headers and body adjusted";
    }
