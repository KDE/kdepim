...
void FlatCollectionProxyModel::setAncestorSeparator( QString s)
{
    d->ancestorSeparator = s;
}
...
QVariant FlatCollectionProxyModel::data(const QModelIndex & index, int role) const
{
  if ( role == Qt::DisplayRole ) {
    ...
    while ( sourceIndex.isValid() ) {
      if ( index.column() == 0 ) {
        name.prepend( sourceIndex.data( role ).toString() + QString( d->ancestorSeparator ) );
      }
      sourceIndex = sourceIndex.parent();
    }
    return name;
