...
: NepomukFeederPlugin(parent)
  //m_indexEncryptedContent(true)
{
  //m_indexEncryptedContent = Settings::self()->indexEncryptedContent() == Settings::NoIndexing;
}
...
void NepomukMailFeeder::updateItem(const Akonadi::Item& item, Nepomuk::SimpleResource& res, Nepomuk::SimpleResourceGraph& graph)
{
  ...
  if ( !msg->body().isEmpty() || !msg->contents().isEmpty() ) {
    /*if ( m_indexEncryptedContent ) {
      MessageViewer::EmptySource source;
      m_nodeHelper.clear();
      MessageViewer::ObjectTreeParser parser(&source, &m_nodeHelper);
      parser.parseObjectTree( msg.get() );
    }*/
